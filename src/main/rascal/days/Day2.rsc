...
bool isSafe(Report rep: [int a, int b, *_]) {
    bool inc = b > a;
    for (<l, r> <- asPairs(rep)) {
        if (l == r) return false;
...
bool isSafe(Report rep: [int a, int b]) = a != b && abs(a - b) <= 3;
bool isSafe(Report rep: [_]) = true;

int a(Input input) = (0 | it + (isSafe(r) ? 1 : 0) | Report r <- input);
